...
public class ImportWarcs extends ToolBase implements WARCRecordMapper
{
  ...
  */
    ...
    throws IOException
  {
    ...
    String mimetype =
      getMimetype(warcData.getMimetype(), this.mimeTypes, url);
   ...
   /* final Header[] headers = rec.getHeaders();
    ...
    final Content content = new Content(url, url, contentBytes, mimetype,
      metaData, getConf());
    datum.setFetchTime(Nutchwax.getDate(warcData.getDate()));
  ...
  {  
      this.arcName = (String) rec.getHeader().getHeaderValue("WARC-Filename");
      this.arcName = this.arcName.replace(".warc.gz", "");   
  }
