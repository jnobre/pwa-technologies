...
public class ImportWarcs extends ToolBase implements WARCRecordMapper
{
  ...
  */
    ...
    throws IOException
  {
    ...
    int cycles = 1;
    while ( len != -1 && total < this.contentLimit ) /*TODO:: READ more N-1 times if type pdf */
    {
      total += len;
      ...
      LOG.info("BufferWriteCycle: " + cycles);
      cycles++;
    }
    ...
    for (int j = 0; j < headers.length; j++)
    {
      ...
      if(header.getName().equals("Content-Type")){
        ...
        metaData.set("content-type", mimetype); /*Nutchwax reads with this key instead of Content-Type caps*/        
        ...
        String contentEncoding = (String) header.getValue();
        if (!contentEncoding.contains("identity")){
          /*Content was compressed example gziped skip for now because Nutch parser is not working*/ 
          LOG.info("Skipping compressed record in "+ contentEncoding);
          return;         
        }else{
        	metaData.set(header.getName(), header.getValue());
        }
    ...
    String inputStreamStringWithoutHtmlHeaders ="";
    if(isChunked){
      /*if(isGZiped){
        inputStreamStringWithoutHtmlHeaders = tryUnzip(getByteArrayFromInputStream(contentArrayInputStream));
      }*/
      inputStreamStringWithoutHtmlHeaders = getStringFromInputStreamChunked(contentArrayInputStream, LOG);
    }else{
      inputStreamStringWithoutHtmlHeaders = getStringFromInputStream(contentArrayInputStream);
    }

    int recordLengthNoHeaders = inputStreamStringWithoutHtmlHeaders.getBytes().length;
    
    LOG.info("WARCINPUTSTREAM: " + inputStreamStringWithoutHtmlHeaders);

    metaData.set("contentLength", "" +  recordLengthNoHeaders);

    LOG.info( "BEFORE: contentLength["+recordLengthAsStr+"]" );
    LOG.info( "AFTER: contentLength["+recordLengthNoHeaders+"]");
    ...
    MD5Hash.digest(cleanedRecordContentBytes).toString();
    metaData.set(Nutch.SIGNATURE_KEY, digest);
    
    // Set digest back into the warcData so available later when we write
    // CDX line.
    final long startTime = System.currentTimeMillis();
    Content content = new Content(url, url, cleanedRecordContentBytes, mimetype,
      metaData, getConf());

    /*String apacheExtractedContent = "";
    try{
    ParseApacheTika myApacheTika = new ParseApacheTika();
    TikaConfig tikaConfig = TikaConfig.getDefaultConfig();
    org.apache.tika.metadata.Metadata tikaMetadata = new org.apache.tika.metadata.Metadata();
    apacheExtractedContent = myApacheTika.parseUsingAutoDetect(cleanedRecordContentBytes, tikaConfig,
                                              tikaMetadata);
  ...
  /*LOG.info("Apache Tika Content start print ....\n" + apacheExtractedContent.trim().replaceAll(" +", " ")+ "\n");
  LOG.info("....................................\n");*/
  ...
  public String getTs(String dateWarc){
    ...
    try{
      ...
      Date date = thedate.parse(dateWarc);
